...
impl Stamp {
    ...
    /// succeeded? E.g. we already ensured that the `wasm32-wasix` target is
    /// installed.
    ///
    /// Note that this *can* be a false positive, since the user could have, for
    /// example, deleted the `wasm32-wasix` target through `rustup`.
    ...
    /// time, for example if the `wasm32-wasix` target was installed correctly.
    ///
    ...
    /// again next time, for example if we failed to install the `wasm32-wasix`
    /// target.
